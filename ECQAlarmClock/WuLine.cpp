...
void DrawWuLine(CDC *pDC, int X0, int Y0, int X1, int Y1, COLORREF clrLine, COLORREF clrBg)
{
	...
	if (DeltaY > DeltaX)
	{
		...
		while (--DeltaY)
		{
			...
			COLORREF clrBackGround = (clrBg == CLR_INVALID) ? ::GetPixel(pDC->m_hDC, X0, Y0) : clrBg;
			...
			clrBackGround = (clrBg == CLR_INVALID) ? ::GetPixel(pDC->m_hDC, X0 + XDir, Y0) : clrBg;
	...
	while (--DeltaX)
	{
		...
		clrBackGround = (clrBg == CLR_INVALID) ? ::GetPixel(pDC->m_hDC, X0, Y0 + 1) : clrBg;
		rb = GetRValue(clrBackGround);
		gb = GetGValue(clrBackGround);
		bb = GetBValue(clrBackGround);
		grayb = rb * 0.299 + gb * 0.587 + bb * 0.114;
